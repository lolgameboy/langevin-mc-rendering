...
class Pss(mi.Integrator):
    ...
    def render(self, scene: mi.Scene, sensor: mi.Sensor, seed: mi.UInt = 0, spp: int = 0, develop: bool = True, evaluate: bool = True) -> mi.TensorXf:
        ...
        print(sample)
        luminance, res, pixel_x, pixel_y = calculate_sample_contribution(sample, scene, pathtracer, cam_transform, plane_size, resolution)
        ...
        while i < N:
            ...
            prop_luminance, prop_res, prop_pixel_x, prop_pixel_y = calculate_sample_contribution(prop_sample, scene, pathtracer, cam_transform, plane_size, resolution)
            
            # Calculate acceptence chance            
            a = dr.min(1, prop_luminance / luminance)

            # print(rng.uniform(mi.Float, 1))
            # TODO Is dit correct?
...
#@dr.freeze
def calculate_sample_contribution(sample, scene, pathtracer, cam_transform, plane_size, resolution):
