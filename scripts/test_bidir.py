...
from utils import render_convergence, render_ref

scene = mi.load_file("../scenes/scene_low_res.xml")
#scene = mi.load_file("../scenes/veach-ajar/scene.xml")
#scene = mi.load_file("../scenes/veach-bidir/scene.xml")
scene_name = "cornell_box_low_res"
...
def render_mc(scene: mi.Scene, sensor: mi.Sensor, N : mi.Int, seed: mi.UInt = 0, spp: int = 0, develop: bool = True, evaluate: bool = True) -> mi.TensorXf:
        ...
        while i < N:
            ...
            luminance, res, pixel_x, pixel_y = calculate_sample_contribution_bidir(sample, scene, cam_transform, plane_size, resolution, max_depth=6)
...
img = render_mc(scene, sensor, 1000000) ** (1.0 / 2.2) # approximate sRGB tonemapping TODO why this needed?
#ref_image = render_convergence(scene, scene_name, 0.001, True) ** (1.0 / 2.2)
ref_image = render_ref(scene, 2**7, 2) ** (1.0 / 2.2)
diffimg = dr.abs(ref_image - img)
print(ref_image[60][30] ** (1.0 / 2.2))
print(img[60][30] ** (1.0 / 2.2))
print(diffimg[60][30] ** (1.0 / 2.2))
rmse = dr.sqrt(dr.mean(dr.square(ref_image - img)))
plt.axis("off")
plt.imshow(img); # approximate sRGB tonemapping TODO why this needed?
plt.figure()
plt.imshow(diffimg)
plt.show()